...
class UseData(object):
    ...
    def hasTable(self):
        ...
        try:
            ...
            sql = 'CREATE TABLE IF NOT EXISTS {0} (id INT NOT NULL AUTO_INCREMENT,srcIp VARCHAR(100) NOT NULL,dstIp VARCHAR(100) NOT NULL,sport INT NOT NULL,dport INT NOT NULL,count int NOT NULL,PRIMARY KEY ( id ),agent VARCHAR(100) NOT NULL)'.format(
                conf.MYSQL_TABLE)
            cursor.execute(sql)
    ...
    '''
    @description: 批量插入
    @param {tuple} 表头 srcIp, dstIp, sport,dport,count,agent
    @param {list} 表值 [(),()]
    '''
    ...
    def insert(self, values):
        '''批量插入'''
        try:
            logger.info("数据库表 %s 数据插入", conf.MYSQL_TABLE)
            connection = self.connection()
            cursor = connection.cursor()
            sql = 'INSERT INTO {0} (srcIp, dstIp, sport,dport,count,agent) VALUES '.format(
                conf.MYSQL_TABLE)
            length = len(values)
    ...
    def clearTable(self):
        try:
            logger.info("数据库表 %s 清空", conf.MYSQL_TABLE)
            connection = self.connection()
            cursor = connection.cursor()
